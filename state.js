...
DiceApp.database =
  JSON.parse(localStorage.getItem("dice_profiles_v2")) ||
...
if (
  ...
  try {
    const oldDb = JSON.parse(localStorage.getItem("dice_profiles"));
    Object.keys(oldDb).forEach((charKey) => {
      if (Array.isArray(oldDb[charKey])) {
        DiceApp.database[charKey] = {
          buttons: oldDb[charKey],
          variables: {},
        };
      }
    });
...
DiceApp.ensureCharacterStructure = function (charName) {
  if (!DiceApp.database[charName])
    DiceApp.database[charName] = { buttons: [], variables: {}, notes: "" };
  if (!DiceApp.database[charName].buttons)
    DiceApp.database[charName].buttons = [];
  if (!DiceApp.database[charName].variables)
    DiceApp.database[charName].variables = {};
  if (typeof DiceApp.database[charName].notes === "undefined")
    DiceApp.database[charName].notes = "";
};

DiceApp.saveToStorage = function () {
  localStorage.setItem("dice_profiles_v2", JSON.stringify(DiceApp.database));
